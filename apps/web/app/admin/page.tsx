...
export default function AdminPage() {
  ...
  async function load() {
    ...
    const res = await fetch(`/api/admin/questions?${params.toString()}`, { headers: adminHeaders() });
    if (res.status === 401) return setQuestions([]);
    setQuestions(await res.json());
  }

  useEffect(() => { load(); }, [approved, category]);
  ...
  async function generate() {
    await fetch("/api/admin/ai/generate", { method: "POST", headers: adminHeaders() });
    await load();
  }
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button onClick={generate} className="rounded-2xl bg-slate-950 px-5 py-3 font-bold text-white">Générer 1 question IA</button>
      </div>

      <div className="mt-6 flex flex-wrap gap-3">
        <input value={adminSecret} onChange={(e) => setAdminSecret(e.target.value)} placeholder="ADMIN_SECRET" className="rounded-xl border px-4 py-2" />
        <button onClick={saveSecret} className="rounded-xl bg-blue-700 px-4 py-2 font-bold text-white">Save secret</button>
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
                  <span>{q.category}</span><span>{q.difficulty}</span><span>{q.createdByAi ? "AI" : "Manual"}</span><span>{q.approved ? "Approved" : "Pending"}</span>
                </div>
                <h2 className="mt-2 text-xl font-black">{q.questionText}</h2>
              </div>
              <div className="flex gap-2">
                {!q.approved && <button onClick={() => patch(q.id, { approved: true })} className="rounded-xl bg-green-600 px-4 py-2 font-bold text-white">Approve</button>}
                {q.approved && <button onClick={() => patch(q.id, { approved: false })} className="rounded-xl bg-amber-500 px-4 py-2 font-bold text-white">Unapprove</button>}
                <button onClick={() => remove(q.id)} className="rounded-xl bg-red-600 px-4 py-2 font-bold text-white">Delete</button>
              </div>
            </div>
            <ol className="mt-4 grid gap-2 md:grid-cols-2">
              {[q.answer1, q.answer2, q.answer3, q.answer4].map((a, index) => (
                <li key={a} className={`rounded-xl border px-3 py-2 ${q.correctAnswer === index + 1 ? "border-green-500 bg-green-50" : "border-slate-200"}`}>{index + 1}. {a}</li>
              ))}
            </ol>
